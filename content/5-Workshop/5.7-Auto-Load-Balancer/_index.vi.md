...
1. Truy cập **AWS Management Console** $\rightarrow$ dịch vụ **EC2**.
2. Tại menu bên trái, cuộn xuống mục **Load Balancing** $\rightarrow$ chọn **Target Groups**.
...
1. Vào **AWS Management Console** $\rightarrow$ chọn dịch vụ **EC2**.
2. Ở menu bên trái, cuộn xuống mục **Load Balancing** $\rightarrow$ chọn **Load Balancers**.
3. Nhấp nút màu cam **Create load balancer**.
4. Chọn loại **Application Load Balancer (ALB)** $\rightarrow$ nhấp nút **Create**.
...
2. **Mappings:** Tích chọn 2 Availability Zones khác nhau:
   * Zone **`ap-southeast-1a`** $\rightarrow$ Chọn Subnet: `fca-public-subnet-a`
   * Zone **`ap-southeast-1b`** $\rightarrow$ Chọn Subnet: `fca-public-subnet-b`
...
1. Tại phần **Listeners and routing**:
   * **Protocol:** `HTTP`
   * **Port:** `80`
2. Tại phần **Default action**, mở danh sách thả xuống chọn **Forward to** $\rightarrow$ chọn **`fca-web-target-group`**.
...
1. Truy cập **AWS Management Console** $\rightarrow$ **EC2** $\rightarrow$ **Security Groups**.
2. Tìm và nhấp chọn Security Group của EC2: **`fca-web-sg`**.
3. Chọn tab **Inbound rules** $\rightarrow$ Bấm **Edit inbound rules**.
4. Cập nhật quy tắc cho cổng `5000`:
   * Xóa quy tắc IPv4 CIDR cũ (nếu có).
   * Bấm **Add rule**:
     * **Type:** `Custom TCP`
     * **Port range:** `5000`
     * **Source:** Chọn **Custom** $\rightarrow$ Nhập/chọn mã Security Group của ALB (**`workshop-alb-sg`**).
...
1. Truy cập **AWS Management Console** $\rightarrow$ **EC2** $\rightarrow$ **Target Groups**.
2. Nhấp chọn **`fca-web-target-group`**.
3. Chọn tab **Targets** $\rightarrow$ nhấp nút **Register targets**.
...
1. Truy cập **AWS Management Console** $\rightarrow$ **EC2**.
